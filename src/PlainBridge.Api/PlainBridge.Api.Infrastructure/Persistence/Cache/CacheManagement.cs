...
public class CacheManagement(ILogger<CacheManagement> _logger, HybridCache _hybridCache) : ICacheManagement
{
    ...
    public async Task<string?> SetGetSubIdTokenAsync(string sub, string value = default!, CancellationToken cancellationToken = default!) => await _hybridCache.GetOrCreateAsync($"subidtoken:{sub}", async ct => value, cancellationToken: cancellationToken);
